...
class TestFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.child = pexpect.spawn(
            f"{TOPDIR}/bin/lshell --config {TOPDIR}/test/lshell-test.conf --strict 1"
        )
    ...
    def test_16_exitcode_with_separator_external_cmd(self):
        ...
        self.child = pexpect.spawn(
            f"{TOPDIR}/bin/lshell "
            f"--config {TOPDIR}/test/lshell-test.conf "
            '--forbidden "[]"'
        )
        self.child.expect(f"{self.user}:~\\$")
    ...
    def test_24_cd_and_command(self):
        ...
        self.child = pexpect.spawn(
            f"{TOPDIR}/bin/lshell " f"--config {TOPDIR}/test/lshell-test.conf"
        )
        self.child.expect(f"{self.user}:~\\$")
    ...
    def test_33_ls_non_existing_directory_and_echo(self):
        ...
        self.child = pexpect.spawn(
            f"{TOPDIR}/bin/lshell --config {TOPDIR}/test/lshell-test.conf"
        )
        self.child.expect(f"{self.user}:~\\$")
    ...
    def test_39_script_execution_with_template(self):
        ...
        with open(wrapper_path, "w") as wrapper:
            wrapper.write(
                f"""#!/bin/bash
exec {TOPDIR}/bin/lshell --config {TOPDIR}/test/lshell-test.conf "$@"
"""
            )
...
*** forbidden path: /"""
    ...
    def test_40_script_execution_with_template_strict(self):
        ...
        with open(wrapper_path, "w") as wrapper:
            wrapper.write(
                f"""#!/bin/bash
exec {TOPDIR}/bin/lshell --config {TOPDIR}/test/lshell-test.conf --strict 1 "$@"
"""
            )
